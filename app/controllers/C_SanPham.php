<?php
    ...
    require_once '../models/redisCache.php';
    ...
    class SanPhamController {
        ...
        private function processInsert() {
            ...
            if ($this -> SanPhamModel -> themSanPham($sanPham, $mausac, $kichthuoc)) {
                ...
                // Lưu thông tin sản phẩm vào cache
                $cache -> set('sanpham_' . $masp, json_encode($product));
        ...
        private function processUpdate() {
            ...
            $cachedProduct = json_decode($cache->get('sanpham_' . $masp), true);
            // Lấy hình ảnh cũ của sản phẩm thông qua cache
            $oldImage = $cachedProduct ? $cachedProduct['hinhanh'] : null;
            ...
            if ($this -> SanPhamModel -> capNhatSanPham($sanPham, $mausac_edit, $kichthuoc_edit)) {
                $updatedImage = $newImage ? $new_filename : $oldImage;
                $product = [
                    'masp' => $masp,
                    'tensp' => $tensp,
                    'gia' => $gia,
                    'soluong' => $soluong,
                    'hinhanh' => $updatedImage,
                    'maloai' => $maloai,
                ];

                // Thay đổi thông tin sản phẩm trong cache
                $cache -> set('sanpham_' . $masp, json_encode($product));
